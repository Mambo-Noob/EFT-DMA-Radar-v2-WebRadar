        ...
        private static bool InitKeyboardForNewWindows()
        {
            ...
            foreach (var csrss in csrssProcesses)
            {
                try
                {
                    ...
                    //no worries ida1337 at it again :3 
                    if (InputManager.currentBuild >= 26100 && InputManager.updateBuildRevision >= 2605)
                    {
                        InputManager.gafAsyncKeyStateExport = userSessionState + 0x3830;
                    }
                    else if (InputManager.currentBuild >= 26100)
                    {
                        InputManager.gafAsyncKeyStateExport = userSessionState + (ulong)(InputManager.updateBuildRevision >= 2314 ? 0x3828 : 0x3820);
                    }
                    else if (InputManager.currentBuild >= 22631 && InputManager.updateBuildRevision >= 3810)
                    {
                        gafAsyncKeyStateExport = userSessionState + 0x36A8;
                    }
                    else
                    {
                        Program.Log("Older windows version detected, Attempting to resolve by offset");
                        gafAsyncKeyStateExport = userSessionState + 0x3690;
                    }
                    ...
                    if (InputManager.gafAsyncKeyStateExport > 0x7FFFFFFFFFFF)
                    {
                        ...
                        Console.WriteLine("Keyboard handler initialized :3");
                        return true;
                    }
                }
                catch { }
